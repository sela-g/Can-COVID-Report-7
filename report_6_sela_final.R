...
#copied from work by Arianne Albert & Gal Av-gay
#written in
...
## Read in NL data
nl <- read.csv("NL_CLEANED_2024-01-21.csv", header = TRUE)
nl$province <- "NL"

## Read in NS data
ns <- read.csv("NS_CLEANED_2024-01-21.csv", header = TRUE)
ns$province <- "NS"
## read in BC data
...
mb.nb.pe <- read.csv("MB_NB_PE_YT_CLEANED_2023-02-27.csv")
mb.nb.pe <- mb.nb.pe %>% 
...
all_combined <- rbind(qc[,which(colnames(qc) %in% all_cols)],
                      ...
                      ns[,which(colnames(ns) %in% all_cols)])
#                      nl[,which(colnames(nl) %in% all_cols)])
...
### CURRENTLY GIVING PROBLEMS SO SKIPPING ### MODE OF COVID 19 ACQUISITION ### 
...
# 
# write.csv(bcmbnbpe, here("bcmbnbpe.csv"))
# 
